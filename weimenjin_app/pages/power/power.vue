			...
			<view class="list" v-if="listStatus"> 
				<view class="item" v-for="(item, index) in dataList" :key="index" @click="showMobile(item)">
					<view class="left-box">
						...
						<image src="../../static/power.png" class="user-img" v-else></image>
						<view class="user-info">
							<view class="flex-box">
								<view class="user-name tourist">{{convertVoltageToPercentage(item.batterypower) }}({{ item.batterypower/1000 }}V)</view>
							</view>
					...
					<view class="right-box">
						<view class="equipment-name">
							<view class="text" v-if="item.lock">{{ item.lock.lock_sn }}</view>
						</view>
					</view>
				</view>
