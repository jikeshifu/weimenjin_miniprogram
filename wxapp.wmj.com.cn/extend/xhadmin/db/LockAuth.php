...
class LockAuth extends Common
{
	...
	protected static $tableName = 'lockauth';   //数据表名
	protected static $pk = 'lockauth_id';   //主键
    ...
    public static function getWhereInfo($where,$field='',$orderby='')
    {
		try{
			empty($orderby) && $orderby = self::$pk.' desc';
			empty($field) && $field = '*';
			$result = Db::name(self::$tableName)->field($field)->where($where)->whereNull("deleted_at")->order($orderby)->find();
		}catch(\Exception $e){
			self::setLog($e->getMessage());
		}
    ...
    {
		try{
			$result = Db::name(self::$tableName)->where($where)->update(["deleted_at"=>date("Y-m-d H:i:s")]);
		}catch(\Exception $e){
			self::setLog($e->getMessage());
    ...
    public static function relateQuery($field,$thisField,$relate_table,$relate_field,$where,$limit,$orderby='')
    {
		try{
			empty($orderby) && $orderby = self::$pk.' desc';
			list($start,$end) = explode(',',$limit);
			$result = Db::name(self::$tableName)->whereNull("a.deleted_at")->field($field)->alias('a')->join($relate_table.' b','a.'.$thisField.'=b.'.$relate_field,"LEFT")->where($where)->limit($start,$end)->order($orderby)->select()->toArray();
    ...
    {
		try{
			$count = Db::name(self::$tableName)->whereNull("a.deleted_at")->field($field)->alias('a')->join($relate_table.' b','a.'.$thisField.'=b.'.$relate_field,"LEFT")->where($where)->count();
		}catch(\Exception $e){
			self::setLog($e->getMessage());
		}
		return $count;
