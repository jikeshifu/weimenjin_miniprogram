...
CodeInfoDlg.collectData = function () {
	this.set('locktype_id').set('locktype_id').set('locktype_name');
};
...
CodeInfoDlg.add = function () {
	 ...
	 var tip = '添加';
	 var ajax = new $ax(Feng.ctxPath + "/LockType/add", function (data) {
	 	if ('00' === data.status) {
...
CodeInfoDlg.update = function () {
	 ...
	 var tip = '修改';
	 var ajax = new $ax(Feng.ctxPath + "/LockType/update", function (data) {
	 	if ('00' === data.status) {
	 		Feng.success(data.msg);
	 		window.parent.CodeGoods.table.refresh();
	 		CodeInfoDlg.close();
