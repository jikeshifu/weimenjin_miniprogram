...
CodeInfoDlg.collectData = function () {
	this.set('umember_id').set('umember_id').set('headimgurl').set('nickname').set('mobile').set('user_id').set('member_id').set('ucreate_time').set('realname').set('remark');
};
...
CodeInfoDlg.authlocks = function () {
	 this.clearData();
	 this.collectData();
	 if (!this.validate()) {
	 	return;
	 ...
	 var status = $("input[name = 'status']:checked").val();
	 var authlocks = '';
	 $('input[name="authlocks"]:checked').each(function(){ 
	 	authlocks += ',' + $(this).val(); 
	 }); 
	  authlocks = authlocks.substr(1); 
	 var tip = '修改';
	 var ajax = new $ax(Feng.ctxPath + "/Umember/authlocks", function (data) {
	 	if ('00' === data.status) {
	 		Feng.success(data.msg);
	 		window.parent.CodeGoods.table.refresh();
	 		CodeInfoDlg.close();
