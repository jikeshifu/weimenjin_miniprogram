...
class LockAuth extends Common
{
    ...
    function getauthlistbylockid()
    {
        ...
        try {
            if ($keyword) {
                $sql = 'select a.*,b.headimgurl,b.nickname,b.mobile,c.lock_name from cd_lockauth as a inner join cd_member as b inner join cd_lock as c where a.member_id=b.member_id and a.deleted_at IS NULL and a.lock_id=c.lock_id and (b.nickname like \'%' . $keyword . '%\' or b.mobile like \'%' . $keyword . '%\' or a.aremark like \'%' . $keyword . '%\')';
            } else {
                $sql = 'select a.*,b.headimgurl,b.nickname,b.mobile,c.lock_name from cd_lockauth as a inner join cd_member as b inner join cd_lock as c where a.member_id=b.member_id  and a.deleted_at IS NULL and a.lock_id=c.lock_id';
            }
    ...
    function applyauth()
    {
        ...
        try {
            ...
            if (1) {
                //查询管理员信息
                $ntsql = 'select a.member_id,b.unionid from cd_lockauth as a inner join cd_member as b on a.member_id=b.member_id where lock_id =' . $data['lock_id'] . ' and auth_isadmin = 1 order by member_id asc limit 10';
                $ntres = db()->query($ntsql);
                //查用户手机号
                $member = db()->name('member')->where(['member_id' => $data['member_id']])->column('mobile');
                $emwhere['member_id'] = $data['member_id'];
                $emfield['realname'] = $data['realname'];
                $emfield['remark'] = $data['remark'];
                $uemfield['urealname'] = $data['realname'];
                $uemfield['uremark'] = $data['remark'];
                $emember = \xhadmin\db\Member::editWhere($emwhere, $emfield);
                $emember = \xhadmin\db\Umember::editWhere($emwhere, $uemfield);
                $senddata['lock_id'] = $data['lock_id'];
                $senddata['lockname'] = $lockdata['lock_name'];
                $senddata['username'] = $data['realname'];
                $senddata['mobile'] = $member[0];
                $senddata['uniondata'] = $ntres;
                wmjSendWechatMsg('examinent', $senddata);
                //   mlog("applylock:");
    ...
    function verifyauth()
    {
        ...
        try {
            ...
            if ($lockauthinfo[0]['member_id'] == $adminmember['member_id']) {
                return json(['status' => $this->errorCode, 'msg' => '不能操作超级管理员']);

            }
            if ($lockauthinfo[0]['member_id'] == $data['auth_member_id']) {
                return json(['status' => $this->errorCode, 'msg' => '不能操作自己']);
            ...
            if ($data['auth_endtime'] == 0) {
                unset($data['auth_endtime']);
            }
            if ($data['remark']) {
                $data['aremark'] = $data['remark'];
                unset($data['remark']);
            }
     ...
     * {"status":" 201","msg":"查询失败"}
     */
    function getauthdebugid()
    {
        $limit = $this->request->post('limit', 20, 'intval');
        $page = $this->request->post('page', 1, 'intval');
        $memberid = 13124;
        //mlog("LockAuth_getauthdebugid_page:" . $page);
    ...
    function getauthlistbymemid()
    {
        ...
        if (!$memberid){
            $memberid =$this->_data["uid"];
        }
        if (!$memberid) return json(['status' => $this->errorCode,'code' => $this->errorCode, 'msg' => 'member_id不能为空']);
        $where = [];
        $where['a.member_id'] = $memberid;
        ...
        try {
            ...
            foreach ($res['list'] as &$value) {
                ...
                $lockQz=mb_substr($value["lock_sn"],0,5);
                if(mb_substr($value["lock_sn"],0,3)=="W89"){
                    $value["pwd_status"]=(int)1;
                }
                $value["showcard"]=(int)0;
                $value["audio"]=(int)0;
                $value["face_status"]=(int)0;

                if ($lockQz=="WMJ62" || $lockQz=="WMJ42" || mb_substr($value["lock_sn"],0,3)=="W89"){

                    $value["showcard"]=(int)1;
                }
                if (mb_substr($value["lock_sn"],0,3)=="W77"){

                    $value["face_status"]=(int)1;
                }
                //语音
                if ( mb_substr($value["lock_sn"],0,3)=="W82" ||$lockQz=="WMJ62" ){

                    $value["audio"]=(int)1;
                }
                $value["cz_status"]=(int)0;
                //操作序列号
                if ( mb_substr($value["lock_sn"],0,3)=="W71" || mb_substr($value["lock_sn"],0,3)=="W72"){
                    $value["cz_status"]=(int)1;
                }

            }
        } catch (\Exception $e) {
            return json(['status' => $this->errorCode,'code' =>  $this->errorCode, 'msg' => $e->getMessage()]);
        }

        return json(['status' => $this->successCode,'code' => 0, 'data' => htmlOutList($res)]);
    }
