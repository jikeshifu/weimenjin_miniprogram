...
class Menu
{
	...
	protected static $tableName = 'menu';   //数据表名
	protected static $pk = "menu_id";       //主键
    ...
    public static function loadList($where=[],$limit='100',$field='*',$orderby=''){	
		try{
			empty($orderby) && $orderby = 'sortid asc';
    ...
    {	
		try{
			$result = Db::name(self::$tableName)->where($where)->column('menu_id');
		}catch(\Exception $e){
			throw new \Exception($e->getMessage());
		}
