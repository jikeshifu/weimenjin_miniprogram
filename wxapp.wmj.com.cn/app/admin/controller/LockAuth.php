...
class LockAuth extends Admin {
	...
	function update(){
		if (!$this->request->isPost()){
			...
			$postField = 'lockauth_id,auth_starttime,auth_endtime,auth_openlimit';
			$data = $this->request->only(explode(',',$postField),'post',null);
			try {
	...
	function delete(){
		...
		return json(['status'=>'00','msg'=>'操作成功',"sql"=>Db::name("lockauth")->getLastSql()]);
	}
...
// 			{
// 			    if ($list[$i]['auth_endtime']==0)
// 			    {
// 			        mlog("auth_list_i:".$list[$i]['auth_endtime']);
//                     $list[$i]['auth_endtime']='-';
//                 }
// 			}
