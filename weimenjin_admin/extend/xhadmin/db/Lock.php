...
class Lock extends Common
{
	...
	protected static $tableName = 'lock';   //数据表名
	protected static $pk = 'lock_id';   //主键
    ...
    public static function loadList($where=[],$limit=100,$field='*',$orderby=''){
		try{
			...
			$result =  Db::name(self::$tableName)->where($where)->whereNull("deleted_at")->field($field)->limit($start,$end)->order($orderby)->select()->toArray();
		}catch(\Exception $e){
    ...
    public static function countList($where){
		...
		try{
			$result = Db::name(self::$tableName)->where($where)->whereNull("deleted_at")->count();
		}catch(\Exception $e){
			self::setLog($e->getMessage());
		}
