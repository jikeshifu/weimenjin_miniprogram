...
namespace app\minilock\http\middleware;
use app\minilock\controller\Jwt;
...
class JwtAuth
{
    ...
    {	
		...
		$jwt = Jwt::getInstance();
		$jwt->setIss(config('my.jwt_iss'))->setAud(config('my.jwt_aud'))->setSecrect(config('my.jwt_secrect'))->setSecrect(config('my.jwt_expire_time'))->setToken($token);
