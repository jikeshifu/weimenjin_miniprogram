	...
	*/
	function getopenlogbylockid()
	{
		...
		try{
		    ...
		    {
		        $sql = 'select a.*,b.headimgurl,b.nickname,b.mobile,c.lock_name,d.lockcard_username,d.lockcard_remark from cd_locklog as a inner join cd_lock as c on a.lock_id=c.lock_id left join cd_member as b on a.member_id=b.member_id left join cd_lockcard as d on a.cardsn=d.lockcard_sn and a.lock_id=d.lock_id where (b.nickname like \'%'.$keyword.'%\' or b.mobile like \'%'.$keyword.'%\')';
		    }
        ...
        foreach ($res["list"] as &$listVo){
            if($listVo["user_name"]){
              $listVo["nickname"]=$listVo["user_name"];
                if(!$listVo["mobile"]){
                    $listVo["mobile"]="00000000000";
                }

              $listVo["headimgurl"]="https://thirdwx.qlogo.cn/mmopen/vi_32/POgEwh4mIHO4nibH0KlMECNjjGxQUq24ZEaGT4poC6icRiccVGKSyXwibcPq4BWmiaIGuG1icwxaQX6grC9VemZoJ8rg/132";
            }
            $listVo["type_name"]=  \app\module\lockServer\LockLog::$type[$listVo["type"]];
        }
	...
	function add()
    {
        ...
        $memberInfo = MemberServer::Info($uidInfo["uid"]);
        //mlog("logadd:".json_encode($LockData,JSON_UNESCAPED_UNICODE),"logadd.txt");
	    ...
	    \app\module\lockServer\LockLog::add($uidInfo["uid"],$lock_id,8,$status,$username,intval($LockData["user_id"]));
	...
	*/
	function getopenlog(){
		...
		try{
		    if(!empty($keyword))
		    {
		        $sql = 'select a.*,b.headimgurl,b.nickname,b.mobile,c.lock_name from cd_locklog as a inner join cd_member as b inner join cd_lock as c where a.member_id=b.member_id and a.lock_id=c.lock_id and (b.nickname like \'%'.$keyword.'%\' or b.mobile like \'%'.$keyword.'%\')';
		    }
		    else
		    {
		        $sql = 'select a.*,b.headimgurl,b.nickname,b.mobile,c.lock_name from cd_locklog as a inner join cd_member as b inner join cd_lock as c where a.member_id=b.member_id and a.lock_id=c.lock_id';
