...
/*start*/
	...
	function index(){
		if (!$this->request->isAjax()){
            ...
            $where['b.member_type'] = ['like',$this->request->param('member_type', '', 'serach_in')];
			if(session('admin.role') <> 1){
	...
	function authlocks(){
		if (!$this->request->isPost()){
			...
			try{
			    ...
			    $lock = db()->name('lock')->where(['user_id'=>$user_id])->whereNull("deleted_at")->field(['lock_id','lock_name'])->select()->toArray();
			    $info = UmemberDb::getInfo($umember_id);
			    $authlocks = db()->name('lockauth')->where(['member_id'=>$info['member_id'],'user_id'=>$info['user_id']])->whereNull("deleted_at")->column('lock_id');
