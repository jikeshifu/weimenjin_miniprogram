...
Page({
  ...
  getmore: async function(page,num,addto){ // addto为0覆盖原有数据，为1在原有数据基础上追加数据
    ...
    if (app.globalData.userid) {
     let getopenlogbylockid=  await  request.HttpPost("LockLog/getopenlogbylockid",{
        limit: num,
        page: page,
      ...
      if(!getopenlogbylockid){
        return
      }

    console.log("getopenlogbylockid",getopenlogbylockid)
      var arrdata = getopenlogbylockid.data.list
      var arr = [];
      if(arrdata && arrdata.length > 0){
        var typearr = ['','扫码开门','点击开门','后台开门','刷卡开门','点击开电','点击关电'];// type为1时为扫码开门，type为2时点击开门，type为3时后台开门，type为4时是刷卡开门
        ...
        for (var i = 0; i < arrdata.length; i++) {
          ...
          tmpobj['mobile'] = phone;
          tmpobj['phone'] = phone.substr(0,1)+"***********"+ phone.substr(10);
          tmpobj['type'] = arrdata[i]['type'];
          tmpobj['typestr'] = arrdata[i]['type_name'];
          if(arrdata[i]['status'] =="1"){
            tmpobj['typestr'] = tmpobj['typestr'] +"成功"
          }else{
            tmpobj['typestr'] = tmpobj['typestr'] +"失败"  
          }
          tmpobj['status'] = arrdata[i]['status'];
          if (arrdata[i]['type']== 4) {
