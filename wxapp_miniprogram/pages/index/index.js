...
var bleW62 = require('../../module/ble/bleW62');
Page({
    data: {
    ...
    onLoad: function (options) {
        ...
        if (options) {
            ...
            if (pos >= 0) {
                ...
                if (paramobj.key) {
                    let UserInfo = request.HttpPost("Member/loginQrCode", {
                        result: paramobj.key,
                    }).catch((err) => {
                        console.log("err", err)
                        wx.hideToast()
    ...
    cz: async function (e) {
        ...
        wx.showActionSheet({
            ...
            success: async function (res) {
                //开电
                if (res.tapIndex == 0) {
                    let electricityStart = await request.HttpPost("device.Device/electricityStart", {"lock_id": lock_id})
                    if (electricityStart) {
                        wx.showToast({
                            title: electricityStart.msg,
                ...
                let electricityStop = await request.HttpPost("device.Device/electricityStop", {"lock_id": lock_id})
                if (electricityStop) {
                    wx.showToast({
                        title: electricityStop.msg,
                        icon: 'none',
                        mask: true, // 防止触摸穿透
                        duration: 2000
                    });
    ...
    getmore: async function (page, num, addto) { // addto为0覆盖原有数据，为1在原有数据基础上追加数据
        ...
        let getauthlistbymemid = await request.HttpPost("LockAuth/getauthlistbymemid", {
        ...
        if (!getauthlistbymemid) {
            return
        }
        console.log("getauthlistbymemid", getauthlistbymemid)
        ...
        var arrdata = getauthlistbymemid.data.list
        if (arrdata.length > 0) {
            ...
            for (var i = 0; i < arrdata.length; i++) {
                ...
                tmpobj['showcard'] = Number(arrdata[i]['showcard']);

                tmpobj['online'] = arrdata[i]['online'] == 1 ? true : false;
                tmpobj['lock_id'] = arrdata[i]['lock_id'];
                tmpobj['lock_sn'] = arrdata[i]['lock_sn'];
                tmpobj['ble_status'] = 0;
                if (tmpobj['lock_sn'].indexOf('WMJ62') > -1 || tmpobj['lock_sn'].indexOf('WMJ62') > -1 ) {
                  if(!tmpobj['online'] ){
                    tmpobj['ble_status'] = 1;
                  }
                ...
                tmpobj['pwd_status'] = arrdata[i]['pwd_status'];
                tmpobj['face_status'] = arrdata[i]['face_status'];
                tmpobj['cz_status'] = arrdata[i]['cz_status'];
                tmpobj['audio'] = arrdata[i]['audio'];
    ...
    opendoor: async function (e) {
        console.log("e:", e)
        console.log("online:", e.currentTarget.dataset.deviceinfo.online)

        let lock_sn=e.currentTarget.dataset.deviceinfo.lock_sn

        if (lock_sn.indexOf('WMJ62') > -1 && !e.currentTarget.dataset.deviceinfo.online) {

            await bleW62.OpenLock(lock_sn,e.currentTarget.dataset.deviceinfo.lock_id)
            return
        }
    ...
    w71info: function (e) {
        var lock_id = e.currentTarget.dataset['lockid']; //
        wx.navigateTo({
            url: '../w71info/index?lock_id=' + lock_id
        })
    },
    cardlist: function (e) {
        var lock_id = e.currentTarget.dataset['lockid']; //
        ...
        var lock_id = e.currentTarget.dataset['lockid']; //
        wx.navigateTo({
            url: '../face/faceList/faceList?lock_id=' + lock_id
        })
    },
    pwdList: function (e) {
        var lock_id = e.currentTarget.dataset['lockid']; //
        wx.navigateTo({
            url: '../pwdList/pwdList?lock_id=' + lock_id
        })
    }, fingerList: function (e) {
        var lock_id = e.currentTarget.dataset['lockid']; //
        wx.navigateTo({
            url: '../fingerList/fingerList?lock_id=' + lock_id
        })
    },
    addcard: function (e) {
        var adminid = e.currentTarget.dataset['adminid']; // 管理员user_id的值
        ...
        wx.showModal({
            ...
            success: async (res) => {
                if (res.confirm) {
                    ...
                    let deleteRes = await request.HttpPost("Lock/delete", {
                        lock_ids: id,
                        lock_id: id,
