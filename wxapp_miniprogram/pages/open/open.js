...
Page({
  ...
  getLock:async function () {
    ...
    var lock = info.data;
  //   if (lock.lock_sn.indexOf('WMJ62') > -1 && !lock.online) {
  //
  //     await bleW62.OpenLock(lock.lock_sn,lock.lock_id)
  //     return
  // }
  ...
  doOpen:async function (adminid,lock_id) {
    ...
    let opendoor= await request.HttpPost("Lock/opendoor",jsondata).catch((err)=>{
      console.log("err",err)

    if (err.opendoor_status=='202') {
        that.setData({
          isBindPhone:true
        })
      }else if (err.opendoor_status=='203') {
        that.setData({
          apply: true,
          auth_status: 0
        })
      }else if (err.opendoor_status=='204') {
        that.setData({
          apply: true,
          auth_status: 1
        })
    ...
    app.globalData.successimg = app.globalData.domain+opendoor.successimg;
    app.globalData.successadimg = app.globalData.domain+opendoor.successadimg;
    app.globalData.openadurl = opendoor.openadurl;
    app.globalData.adnum = opendoor.adnum;
    app.globalData.qrshowminiad = opendoor.qrshowminiad;
    wx.redirectTo({
