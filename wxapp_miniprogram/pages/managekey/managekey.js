...
Page({
  ...
  getmore:async function(page,num,addto){ // addto为0覆盖原有数据，为1在原有数据基础上追加数据
   ...
   let getauthlistbylockid=  await request.HttpPost("LockAuth/getauthlistbylockid",{
        lock_id: that.data.lock_id,
        auth_status: that.data.auth_status,
        keyword: that.data.keyword,
        limit: num,
      ...
      if(!getauthlistbylockid){
        return
      }

      var arrdata = getauthlistbylockid.data.list
      var arr = [];
      if(arrdata && arrdata.length > 0){
        ...
        for (var i = 0; i < arrdata.length; i++) {
          ...
          tmpobj['aremark'] = arrdata[i]['aremark'];
