...
Page({
  ...
  onShow:function () {
    ...
    if (app.globalData.userInfo != null) {
      avatarUrl = app.globalData.userInfo.headimgurl ? app.globalData.userInfo.headimgurl : app.globalData.avatarUrl;
      nickName = app.globalData.userInfo.nickname ? app.globalData.userInfo.nickname : app.globalData.nickname;
    }

    var tmpphone =    app.globalData.userInfo.mobile;
    let mobile =""
    if(tmpphone){
       mobile = tmpphone.substr(0,1)+"***********"+ tmpphone.substr(10)
    }
    that.setData({
  ...
  onLoad: function () {
    ...
    if (app.globalData.userInfo) {
      ...
      that.setData({
        phone: app.globalData.phone,
        jiamiphone: tmpphone.substr(0,1)+"***********"+ tmpphone.substr(10),
  ...
  adduser: function() {
    ...
    loginScanCode:async  function() {
    wx.scanCode({
      success :async function (res) {
               let electricityStart = await request.HttpPost("Member/loginQrCode",{"result":res.result}).catch(function(err){
                wx.showToast({
                  icon: 'none',
                  title: '登录失败',
                  mask: true, // 防止触摸穿透
  ...
  deviceNetwork: function() {
    wx.navigateTo({
      url: '../deviceNetwork/index'
    });
  },
  batchNetwork: function() {
    wx.navigateTo({
      url: '../batchNetwork/index'
    });
  },
  goWifi: function() {
    wx.navigateTo({
