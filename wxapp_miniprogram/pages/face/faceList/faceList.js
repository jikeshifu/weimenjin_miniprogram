...
Page({
      ...
      wx.showActionSheet({
        itemList: ['添加人脸'],
         success  : async(res)=>{
          ...
          if(res.tapIndex ==0){
            wx.navigateTo({
              url: '../../face/faceAdd/faceAdd?lock_id=' + _this.data.formData.lock_id
          })
            return
          }
    ...
    pwdDel:async function(e){
        ...
        wx.showModal({
            ...
            success : async function(res) {
                if (res.confirm) {
                    let delPwdRes = await request.HttpPost("device.Face/del", {
                        face_id:pwd_id,
    ...
    list: async function () {
        let list = await request.HttpPost("device.Face/list", this.data.formData)
        if (list) {
            this.setData({
                List:list.data,
