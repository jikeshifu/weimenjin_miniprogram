...
超级管理员进入页面后会自动检测新版本，检测到更新后点击“立即升级”即可。升级前系统会自动备份代码和数据库，更新包统一放在演示平台 `/updates/` 目录下。更新包按完整后台主版本生成，升级时会保留数据库连接配置、上传文件、二维码、证书、运行缓存和历史更新包。

完整更新策略见 [docs/ONLINE_UPDATE_STRATEGY.md](docs/ONLINE_UPDATE_STRATEGY.md)。
